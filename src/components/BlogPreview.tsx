    ...
    <section className="bg-black py-10 md:py-28 px-4 sm:px-6 border-t border-[#40424D]">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-end justify-between mb-8 md:mb-12">
          <div>
            ...
            <h2 className="text-display text-3xl md:text-4xl">
              Latest{" "}
              <span className="text-display-secondary">insights.</span>
            </h2>
          </div>
          <Link
            href="/blog"
            className="hidden md:flex items-center gap-2 text-sm font-medium text-[#9DA2B3] hover:text-[#EDEFF7] transition-colors"
          >
            All posts
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px rounded-xl overflow-hidden">
          {posts.map((post) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group block bg-[#1E1E24] hover:bg-[#252530] transition-colors duration-200 p-6 md:p-8 border border-[#40424D]/30"
            >
              ...
              <h3 className="text-base font-bold text-[#EDEFF7] mb-3 leading-snug group-hover:text-white transition-colors">
                {post.title}
              </h3>
              <p className="text-sm text-[#9DA2B3] leading-relaxed line-clamp-2 mb-5">
                {post.excerpt}
              </p>
        ...
        <div className="mt-6 md:hidden">
          <Link href="/blog" className="flex items-center gap-2 text-sm font-medium text-[#9DA2B3] hover:text-[#EDEFF7] transition-colors">
            All posts
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
            </svg>
